...
{
    public class GetUnloadOrdersCommand : Command
    {
        public override string[] Names { get; set; } = new string[] { "getunloadorders" };

        public override async void Execute(Message message, TelegramBotClient client)
        {
            int days;
            if (message.Text.Contains("getunloadorders30"))
            {
                days = 30;
            }
            else if (message.Text.Contains("getunloadorders90"))
            {
                days = 90;
            }            
            else if (message.Text.Contains("getunloadorders180"))
            {
                days = 90;
            }
            else
            {
                await client.SendTextMessageAsync(message.Chat.Id, $"Неверная команда");
                return;
            }
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(fi))
            {
                ...
                foreach (var j in cells)
                {
                    ...
                    {
                        var status = cells[i, 4].Value?.ToString() ?? "выгружен";
                        ...
                        DateTime nuznho = DateTime.Parse(dataZagruz).AddDays((double)days);
                        if (nuznho.Date < DateTime.Today.Date)
                            continue;
                ...
                if (finded == false)
                {
                    await client.SendTextMessageAsync(message.Chat.Id, $"Мы не нашли ваши выгруженные заказы.", replyToMessageId: message.MessageId);
                }
                //Save your file
                excelPackage.Save();
            }
